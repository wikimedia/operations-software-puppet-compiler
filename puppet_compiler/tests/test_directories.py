...
class TestHostFiles(unittest.TestCase):
    ...
    def test_file_for(self):
        ...
        self.assertEqual(
            self.hostfiles.file_for("change", "diff"), "/mnt/jenkins-workspace/19/diffs/test.example.com.diff"
        )
        self.assertEqual(
            self.hostfiles.file_for("test", "diff"), "/mnt/jenkins-workspace/19/diffs/test.example.com-test.diff"
        )
