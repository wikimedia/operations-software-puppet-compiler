...
class TestHostWorker(unittest.TestCase):
    ...
    @mock.patch(
        "puppet_compiler.utils.facts_file",
        mock.MagicMock(return_value="/var/lib/catalog-differ/puppet/yaml/facts/test.example.com.yaml"),
    )
    def test_facts_file(self, isfile):
        isfile.return_value = True
        self.assertEqual(self.hw.facts_file(), "/var/lib/catalog-differ/puppet/yaml/facts/test.example.com.yaml")
    ...
    def test_compile_all(self, mocker):
        ...
        calls = [
            mock.call("test.example.com", "prod", "/var/lib/catalog-differ/puppet", None),
            mock.call("test.example.com", "change", "/var/lib/catalog-differ/puppet", None),
        ]
    ...
    def test_make_diff(self, puppetcatalog_mock):
        ...
        puppetcatalog_mock.assert_has_calls(
            [
                mock.call("/mnt/jenkins-workspace/19/production/catalogs/test.example.com.pson"),
                mock.call("/mnt/jenkins-workspace/19/change/catalogs/test.example.com.pson"),
            ]
        )
    ...
    def test_run_host(self):
        ...
        fname = os.path.join(self.fixtures, "puppet_var", "yaml", "facts", "test.eqiad.wmnet")
